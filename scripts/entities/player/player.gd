...
func calculate_gravity() -> float:
	return get_default_gravity() * (
			...
			else down_held_gravity_ratio if Input.is_action_pressed("move_down") and velocity.y > 0
			else 1.0
	)

func calculate_gravity_limit() -> float:
	return gravity_limit * (
			down_held_gravity_ratio if Input.is_action_pressed("move_down") and velocity.y > 0
			else 1.0
	)
...
func calculate_wall_slide_speed() -> float:
	return max_wall_slide_speed * (
			down_held_wall_slide_ratio if Input.is_action_pressed("move_down")
			else 1.0
	)
